...
// Returns the smallest positive element of roots, or a negative number if all
// are negative.
...
float DotProduct(Vec2 p1, Vec2 p2) { return p1.x * p2.x + p1.y * p2.y; }
...
float Vec2::theta() const { return atan2(y, x); }

float Vec2::MagnitudeSquared() const { return x * x + y * y; }

float Vec2::Magnitude() const { return sqrt(MagnitudeSquared()); }

// || start + t * direction - target || = r1 + r2, and then solve for t
float DistanceToImpact(Circle start, Vec2 direction, Circle target) {
  if ((start.p - target.p).MagnitudeSquared() <=
      (start.r + target.r) * (start.r + target.r)) {
    return 0;
  }
...
float DistanceToImpact(Circle start, Vec2 direction, LineSegment target) {
  ...
  float kNumeratorConstants =
      y2 * x0 - y1 * x0 - x2 * y0 + x1 * y0 + x2 * y1 - y2 * x1;
  float kNumeratorLinearTerms =
  ...
  float projected =
      DotProduct(u, Vec2{x0 + x_dir * t, y0 + y_dir * t} - target.p1);
  ...
  return dist_from_roots(
      {dist_to_segment,
       DistanceToImpact(start, direction, Circle{target.p1, 0}),
       DistanceToImpact(start, direction, Circle{target.p2, 0})});
}
